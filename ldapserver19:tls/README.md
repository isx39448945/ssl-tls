...
```
[munlai@localhost ldapserver19:tls]$ openssl genrsa -out key_server.pem
Generating RSA private key, 2048 bit long modulus (2 primes)
.......+++++
...+++++
e is 65537 (0x010001)

[munlai@localhost ldapserver19:tls]$ openssl genrsa -out key_CA.pem
Generating RSA private key, 2048 bit long modulus (2 primes)
.......................+++++
.....................................+++++
e is 65537 (0x010001)
...
```
[munlai@localhost ldapserver19:tls]$ openssl req -new -x509 -nodes -sha1 -days 365 -key key_CA.pem -out certif_CA.pem
...
```
[munlai@localhost ldapserver19:tls]$ openssl req -new -key key_server.pem -out certif_server.pem
You are about to be asked to enter information that will be incorporated
into your certificate request.
What you are about to enter is what is called a Distinguished Name or a DN.
There are quite a few fields but you can leave some blank
...
```
[munlai@localhost ldapserver19:tls]$ cat ca.conf
...
```
[munlai@localhost keys]$ openssl x509 -CA certif_CA.pem -CAkey key_CA.pem -req -in certif_server.pem -days 365 -sha1 -extfile ca.conf -CAcreateserial -out crt_server.pem
Signature ok
...
## Fitxer ldap.conf
```
# Comentem o eliminem la següent linea:
...
## Consultes
```
[munlai@localhost ldapserver19:tls]$ ldapsearch -x -LLL -ZZ dn | head
dn: dc=edt,dc=org
...
[munlai@localhost ldapserver19:tls]$ ldapsearch -x -LLL -ZZ -h ldap.edt.org -b 'dc=edt,dc=org' dn | head
dn: dc=edt,dc=org
...
[munlai@localhost ldapserver19:tls]$ ldapsearch -x -LLL -H ldaps://ldap.edt.org dn | head
dn: dc=edt,dc=org
...
[munlai@localhost ldapserver19:tls]$ openssl s_client -connect ldap.edt.org:636
CONNECTED(00000003)
